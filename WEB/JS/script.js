...
qs('#pedidoForm')?.addEventListener('submit', e=>{
  e.preventDefault();
  if(carrinho.length===0){toast('Adicione pelo menos um produto.'); return;}
  const form = new FormData(e.currentTarget);
  const codigoAtivo = form.get('codigoSeguranca') === 'on';
  const total = carrinho.reduce((s,item)=>s+(item.preco*item.qtd),0);
  pedidos.unshift({
    id:`EX-${1050+pedidos.length}`,
    cliente:form.get('cliente'),
    entregador:'Aguardando associação',
    status:'Pedido recebido',
    saida:'-', entrega:'-', total, codigo:codigoAtivo
  });
  qs('#securityPreview').textContent = codigoAtivo ? `Código ${codigoAleatorio()}` : 'Desativado pelo cliente';
  carrinho=[]; renderCarrinho(); renderPedidos(); atualizarKPIs(); e.currentTarget.reset();
